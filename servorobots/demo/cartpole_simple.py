# while in the top servorobot folder, run
# python3 -m servorobots.demo.cartpole_simple
    ...
    def __init__(self, name, ob_space, ac_space, take_weights_here=None):
        ...
        with tf.variable_scope(name):
            obs_tuple = [
                ...
                tf.placeholder(tf.float32,        (None, 4), name="obs1"),
            ]
            ...
            dense1_w = tf.get_variable("dense1_w", [4,32])
            dense1_b = tf.get_variable("dense1_b", [32])
            x = tf.matmul(x, dense1_w) + dense1_b
            x = tf.nn.relu(x)
            dense2_w = tf.get_variable("dense2_w", [32,32])
            dense2_b = tf.get_variable("dense2_b", [32])
            x = tf.matmul(x, dense2_w) + dense2_b
            x = tf.nn.relu(x)
            final_w = tf.get_variable("final_w", [32,1])
            final_b = tf.get_variable("final_b", [1])
        ...
        self.assigns = [
            ...
            (   final_w, take_weights_here["wf"]),
            (   final_b, take_weights_here["bf"]),
        ]
...
def demo_run():
    ...
    env = gym.make('ServobulletInvertedPendulum-play-v0')
